...
struct MicSample { float t; float value; };   // czas i wartosc
        ...
        if (hitMic) {
            ...
            if (wid >= 0 && wid < (int)gWinPackets.size()) {
                ...
                for (size_t j = 0; j < wp.times.size(); ++j) {
                    ...
                    if (gWinIdx == gWinPackets.size() - 1) val = val;
